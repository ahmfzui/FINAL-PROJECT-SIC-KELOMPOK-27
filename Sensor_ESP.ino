...
const char *WIFI_SSID = "FBTU_WIFI";
const char *WIFI_PASS = "@PSB2023";
...
#define RELAY_MOTOR 32
#define PH_pin 35

// Global objects
DHT dht(DHTPIN, DHTTYPE);

const char* FLASK_URL = "http://192.168.1.6:5000/get_latest_temp_settings";
const char* FLASK_TIME_URL = "http://192.168.1.6:5000/get_latest_clock_settings";
...
bool get_latest_clock_settings(String &alarm_time) {
    HTTPClient http;
...
void send_data_to_ubidots(float temperature, float humidity, int air_quality, float PH) {
    HTTPClient http;
...
void setup() {
    ...
    digitalWrite(RELAY_MOTOR, LOW); // Set default to HIGH to keep motor off
...
void loop() {
    ...
    String alarm_time;
    if (!get_latest_clock_settings(alarm_time)) {
        Serial.println("Failed to retrieve clock settings.");
    } else {
        // Menampilkan waktu alarm yang diambil dari Flask
    ...
    parse_alarm_time(alarm_time, alarm_hour, alarm_minute);
    ...
    if (current_hour == alarm_hour && current_minute == alarm_minute) {
        Serial.println("Waktu alarm! Menyalakan motor.");
        digitalWrite(RELAY_MOTOR, LOW); // Nyalakan motor
        delay(10000);
        digitalWrite(RELAY_MOTOR, HIGH);
    } 
    else {
        digitalWrite(RELAY_MOTOR, HIGH); // Matikan motor jika tidak dalam waktu alarm
    }
    ...
    send_data_to_ubidots(temperature, humidity, air_quality, sensor_ph);
